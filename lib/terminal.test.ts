...
describe('Scrollback Viewport Stability', () => {
  ...
  test('new output preserves nearby history when the scrollback buffer prunes old pages', async () => {
    if (!container) return;

    const term = await createIsolatedTerminal({ cols: 40, rows: 5, scrollback: 1 });
    term.open(container);
